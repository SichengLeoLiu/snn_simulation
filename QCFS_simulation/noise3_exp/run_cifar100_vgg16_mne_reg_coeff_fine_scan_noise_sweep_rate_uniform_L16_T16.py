"""
CIFAR-100 VGG16：mne_l2 reg_coeff 精细扫描 + weight_decay 对比 + 噪声注入。

方案 C：输出层（无 IF）不参与 MNE-L2。
精细扫描 reg_coeff ∈ {3e-5, 1e-5, 3e-4, 1e-4}，每个训练后做 rate_uniform 噪声扫描，
并与 weight_decay 基线画在同一张折线图；结尾输出各模型 sigma=0 测试结果。
"""
import csv
import subprocess
import sys
...
OUT = (
    ROOT
    / "noise3_exp"
    / "cifar100_vgg16_mne_reg_coeff_fine_scan_noise_sweep_rate_uniform_L16_T16"
)
...
DATASET = "cifar100"
SEED = 42
MNE_REG_COEFFS = [3e-5, 1e-5, 3e-4, 1e-4]
LVAL = 16
TVAL = 16
...
LINE_STYLES = {
    ...
    "mne_l2:3em05": {"color": "#aec7e8", "label": "mne_l2 rc=3e-5"},
    "mne_l2:1em05": {"color": "#17becf", "label": "mne_l2 rc=1e-5"},
    "mne_l2:3em04": {"color": "#98df8a", "label": "mne_l2 rc=3e-4"},
...
def plot_results(curves: dict[str, list[tuple[float, float]]]) -> None:
    ...
    order = []
    if INCLUDE_WD:
        order.append("weight_decay")
    order.extend(method_key("mne_l2", c) for c in MNE_REG_COEFFS)
    ...
    for no_caption in (False, True):
        ...
        if not no_caption:
            ax.set_title(
                f"CIFAR-100 {ARCH} mne_l2 reg_coeff fine scan vs wd "
                f"(seed={SEED}, L={LVAL}, T={TVAL}, {IF_MODE}, {SCHEME_TAG})"
            )
        fig.tight_layout()
        suffix = "_no_caption" if no_caption else ""
        fig.savefig(
            OUT / f"cifar100_{ARCH}_mne_reg_coeff_fine_scan_noise_sweep{suffix}.png"
        )
        plt.close(fig)
...
def main() -> None:
    ...
    raw_csv = OUT / "cifar100_vgg16_mne_reg_coeff_fine_scan_noise_sweep_raw.csv"
    ...
    for rec in records:
        ...
        print(f"{rec['method']:<22} {rc_str:<10} {acc0:>12.3f}", flush=True)
        summary_rows.append(
            {
                "method": rec["method"],
                "regularizer": rec["regularizer"],
    ...
    print("-" * 48, flush=True)
    print(
        f"Best sigma=0: {best_overall['method']}  acc={best_overall['acc_sigma0']}",
        flush=True,
    )

    summary_csv = OUT / "cifar100_vgg16_mne_reg_coeff_fine_scan_best_test_summary.csv"
    ...
    print(
        f"[DONE] plot: {OUT / 'cifar100_vgg16_mne_reg_coeff_fine_scan_noise_sweep_no_caption.png'}",
        flush=True,
    )
