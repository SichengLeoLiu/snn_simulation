...
用法：
  ...
  python noise3_exp/run_fc3_strict_seed_mne_l2_wd_noise_sweep_rate_uniform_L16_T16.py --plot-only
  python noise3_exp/run_fc3_strict_seed_mne_l2_wd_noise_sweep_rate_uniform_L16_T16.py --copy-important
...
def important_plot_name(arch: str, no_caption: bool) -> str:
    suffix = "_no_caption" if no_caption else ""
    return f"strict_seed_train_{arch}_rate_uniform_noise_sweep_mean_std_lineplot{suffix}.png"
    ...
    for h in h_list:
        ...
        for no_caption in (False, True):
            ...
            if copy_important:
                ...
                dest = IMPORTANT_RESULTS / important_plot_name(arch, no_caption)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="fc3 strict-seed mne_l2+wd rate_uniform + 合并 mean±std 图"
    )
    p.add_argument(
        "--h-list",
        type=int,
    ...
    p.add_argument(
        "--copy-important",
        action="store_true",
        help="复制 PNG 到 important results/（文件名含 rate_uniform，不覆盖 normal 图）",
    )
    return p.parse_args()
...
def main() -> None:
    ...
    print(
        f"\n=== fc3 strict-seed mne_l2+wd rate_uniform (rc={MNE_RC}, wd={WD_COMBO}) ===",
        flush=True,
    )
