"""
CIFAR-100 VGG16：mne_l2 + weight_decay 组合 vs 单独使用 + 噪声注入对比。

方案 C：输出层（无 IF）不参与 MNE-L2。
对比：
  - weight_decay only (wd=5e-4)
  - mne_l2 only (rc=1e-4，精细扫描最优)
  - mne_l2 + wd 组合若干组
每个模型训练后做 rate_uniform 噪声扫描，画在同一张折线图。
"""
import csv
import subprocess
import sys
...
# (label, reg_coeff, weight_decay) — reg_coeff=None 表示纯 wd
EXPERIMENTS = [
    ("weight_decay", None, 5e-4),
    ("mne_l2 rc=1e-4", 1e-4, 0.0),
    ("mne_l2+wd rc=1e-4 wd=5e-4", 1e-4, 5e-4),
    ("mne_l2+wd rc=1e-4 wd=1e-4", 1e-4, 1e-4),
    ("mne_l2+wd rc=3e-5 wd=5e-4", 3e-5, 5e-4),
    ("mne_l2+wd rc=1e-5 wd=5e-4", 1e-5, 5e-4),
]

LINE_STYLES = {
    "weight_decay": {"color": "#ff7f0e", "label": "weight_decay"},
    "mne_l2 rc=1e-4": {"color": "#1f77b4", "label": "mne_l2 rc=1e-4"},
    "mne_l2+wd rc=1e-4 wd=5e-4": {"color": "#2ca02c", "label": "mne_l2+wd rc=1e-4 wd=5e-4"},
    "mne_l2+wd rc=1e-4 wd=1e-4": {"color": "#98df8a", "label": "mne_l2+wd rc=1e-4 wd=1e-4"},
    "mne_l2+wd rc=3e-5 wd=5e-4": {"color": "#9467bd", "label": "mne_l2+wd rc=3e-5 wd=5e-4"},
    "mne_l2+wd rc=1e-5 wd=5e-4": {"color": "#17becf", "label": "mne_l2+wd rc=1e-5 wd=5e-4"},
}
...
def plot_results(curves: dict[str, list[tuple[float, float]]]) -> None:
    ...
    for no_caption in (False, True):
        ...
        if not no_caption:
            ax.set_title(
                f"CIFAR-100 {ARCH} mne_l2+wd combo vs single "
                f"(seed={SEED}, L={LVAL}, T={TVAL}, {IF_MODE}, {SCHEME_TAG})"
            )
        fig.tight_layout()
        suffix = "_no_caption" if no_caption else ""
        fig.savefig(OUT / f"cifar100_{ARCH}_mne_l2_wd_combo_noise_sweep{suffix}.png")
        plt.close(fig)
...
def main() -> None:
    ...
    raw_csv = OUT / "cifar100_vgg16_mne_l2_wd_combo_noise_sweep_raw.csv"
    ...
    summary_csv = OUT / "cifar100_vgg16_mne_l2_wd_combo_best_test_summary.csv"
    ...
    print(
        f"[DONE] plot: {OUT / 'cifar100_vgg16_mne_l2_wd_combo_noise_sweep_no_caption.png'}",
        flush=True,
    )
