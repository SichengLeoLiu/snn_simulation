"""
ImageNet ResNet-18：mne_l2 / weight_decay / mne_l2+wd 三路对比 + 噪声注入。

对比（参数延续 CIFAR 最优组合思路）：
  - weight_decay only (wd=1e-4，ImageNet ResNet 常用)
  - mne_l2 only (rc=1e-4)
  - mne_l2+wd (rc=1e-4, wd=1e-4)

训练：L=16, T=0（ANN 训练），Cosine LR；测试：T=16, rate_uniform。
方案 C：无匹配 IF 的层（含 fc、BasicBlock 第二 Conv）不参与 MNE-L2。

环境变量（可选）：
  IMAGENET_EPOCHS  默认 90（完整训练）；试点可设 30
  IMAGENET_BATCH   默认 128（需约 16GB 显存；12GB 卡可设 64）

数据：HuggingFace imagenet-1k；Gadi 上自动缓存到 /scratch/gs14/sl9144/huggingface。
"""
import argparse
import csv
import os
...
def test_noise_sweep(label: str, ckpt: Path, first_layer_noise_position: str) -> Path:
...
def plot_results(curves: dict[str, list[tuple[float, float]]]) -> None:
    ...
    for no_caption in (False, True):
        ...
        if not no_caption:
            ax.set_title(
                f"ImageNet {ARCH} wd vs mne_l2 vs mne_l2+wd "
                f"(seed={SEED}, L={LVAL}, T={TVAL}, {IF_MODE}, {SCHEME_TAG})"
            )
        fig.tight_layout()
        suffix = "_no_caption" if no_caption else ""
        fig.savefig(OUT / f"imagenet_{ARCH}_mne_l2_wd_combo_noise_sweep{suffix}.png")
        plt.close(fig)


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="ImageNet ResNet18 三路正则噪声扫描（wd / mne_l2 / mne_l2+wd）"
    )
    p.add_argument(
...
def main() -> None:
    ...
    for label, reg_coeff, wd in EXPERIMENTS:
        ...
        mat = test_noise_sweep(label, ckpt, args.first_layer_noise_position)
        records.append(
            {
                "label": label,
    ...
    print(
        f"[DONE] plot: {OUT / 'imagenet_resnet18_mne_l2_wd_combo_noise_sweep_no_caption.png'}",
        flush=True,
    )
